...
function GhostSVG({ level, size = 60 }: { level: AlertLevel; size?: number }) {
  ...
  return (
    <svg width={size} height={size} viewBox="0 0 100 120" fill="none"
      ...
      {level === 'danger'  && <path d="M42 68 Q50 78 58 68" stroke="#1a0a2e" strokeWidth="3" fill="none" strokeLinecap="round" />}
...
function AlertPopup({ event, onDismiss }: { event: SecurityEvent; onDismiss: () => void }) {
  const colors: Record<AlertLevel, string> = { safe: '#7b6fff', warning: '#ffaa00', danger: '#ff3366' };
...
// ── GHOST DASHBOARD ──────────────────────────────────────────
function GhostDashboard({ events, level, isOpen, onClose, stats }: {
  ...
  const levelColors: Record<AlertLevel, string> = { safe: '#7b6fff', warning: '#ffaa00', danger: '#ff3366' };
        ...
        {events.length === 0 ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
            {events.slice(0, 20).map(ev => (
              <div key={ev.id} style={{
                background: '#050a10',
                border: `1px solid ${evColors[ev.level]}22`,
...
export function GhostAgent() {
  ...
  const pushEvent = useCallback((ev: Omit<SecurityEvent, 'id' | 'timestamp'>) => {
    const fullEv: SecurityEvent = { ...ev, id: Math.random().toString(36).slice(2), timestamp: Date.now() };
    ...
    else if (ev.level === 'warning') setLevel(prev => prev === 'safe' ? 'warning' : prev);
    setStats(prev => ({
      ...prev,
      totalTx: prev.totalTx + 1,
      whalesDetected: prev.whalesDetected + (ev.type === 'whale' ? 1 : 0),
      suspiciousTx: prev.suspiciousTx + (['suspicious', 'attack'].includes(ev.type) ? 1 : 0),
    }));
  }, []);

  // Watch real onchain events
  useWatchContractEvent({
    address: VAULT_ADDRESS,
    abi: VAULT_ABI,
    eventName: 'FeesCollected',
    onLogs() {
      pushEvent({ type: 'rebalance', level: 'safe', wallet: randomAddr(), message: 'Fees collected from Uniswap V3 position' });
    },
  });
  ...
  useWatchContractEvent({
    ...
    onLogs() {
      pushEvent({ type: 'rebalance', level: 'warning', wallet: randomAddr(), message: 'Vault rebalanced — new range set by keeper' });
    },
  });
  ...
  useEffect(() => {
    const scenarios = [
      ...
      () => pushEvent({ type: 'suspicious', level: 'warning', wallet: randomAddr(), message: 'Rapid deposit/withdraw cycle detected — possible bot' }),
    ...
    const id = setInterval(() => {
      const r = Math.random();
      if      (r < 0.40) scenarios[0]();
      else if (r < 0.60) scenarios[1]();
      else if (r < 0.75) scenarios[2]();
      else if (r < 0.88) scenarios[3]();
      else                scenarios[4]();
  ...
  useEffect(() => {
    if (level === 'danger')  { const id = setTimeout(() => setLevel('warning'), 15000); return () => clearTimeout(id); }
    if (level === 'warning') { const id = setTimeout(() => setLevel('safe'),    20000); return () => clearTimeout(id); }
  }, [level]);

  const levelColors: Record<AlertLevel, string> = { safe: '#7b6fff', warning: '#ffaa00', danger: '#ff3366' };
  const levelLabels: Record<AlertLevel, string>  = { safe: 'ALL CLEAR', warning: 'MONITORING', danger: 'ALERT' };
  const color = levelColors[level];

  return (
    <>
      <style>{`
        @keyframes ghostFloat  { 0%,100%{transform:translateY(0px) rotate(-1deg)} 50%{transform:translateY(-10px) rotate(1deg)} }
        @keyframes ghostPulse  { 0%,100%{transform:translateY(0px) scale(1)}      50%{transform:translateY(-12px) scale(1.08)} }
        @keyframes ringPulse   { 0%{transform:scale(1);opacity:.6}                100%{transform:scale(2.2);opacity:0} }
        @keyframes badgePop    { 0%{transform:scale(.8)} 50%{transform:scale(1.1)} 100%{transform:scale(1)} }
      `}</style>
      ...
      <div style={{
        ...
        {pulse && (
          <div style={{
            ...
            border: `2px solid ${color}`, animation: 'ringPulse 0.8s ease-out forwards',
          }} />
        )}
        ...
        {events.length > 0 && (
          <div style={{
            ...
            background: color, color: '#050a10', borderRadius: '50%',
            width: '20px', height: '20px',
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            fontFamily: 'var(--mono)', fontSize: '0.55rem', fontWeight: 700,
            animation: 'badgePop 0.3s ease', boxShadow: `0 0 8px ${color}`,
          }}>
      ...
      <GhostDashboard
        events={events} level={level}
        isOpen={dashOpen} onClose={() => setDashOpen(false)}
        stats={stats}
      />
    </>
  );
